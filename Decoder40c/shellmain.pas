...
begin
  ...
  if vsl.Count > 1 then
    res := Messagebox(handle, pchar(langini.ReadString('ShlExt', 'multiplefiles', '')), pchar(langini.ReadString('ShlExt', 'multiplecaption', '')), MB_ICONQUESTION or MB_YESNO)
  else
    res := ID_YES;
...
initialization
  ...
  with TRegistry.Create do
    try
      ...
      OpenKey('SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall\(De)Coder_is1', False);
